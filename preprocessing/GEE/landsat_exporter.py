...
class LandsatExporter:
    """
    Class for exporting Landsat images from Google Earth Engine
    """
    
    MS_BANDS = ['BLUE', 'GREEN', 'RED', 'NIR', 'SWIR1', 'SWIR2']
    
    def get_collection(filterpoly: ee.Geometry, start_date: str, end_date: str, collection: int = None, 
                       mask_clouds: bool = True, drop_faulty_l7: bool = False) -> ee.ImageCollection:
        """
        Creates a standardised ee.ImageCollection containing all Landsat images of desired points
        between the desired start and end dates.
        """
    
        landsat_8 = LandsatExporter.init_coll('LANDSAT/LC08/C02/T1_L2', filterpoly, start_date, end_date) # 30 x 30 m/px
        landsat_7 = LandsatExporter.init_coll('LANDSAT/LE07/C02/T1_L2', filterpoly, start_date, end_date) # 30 x 30 m/px
        landsat_5 = LandsatExporter.init_coll('LANDSAT/LT05/C02/T1_L2', filterpoly, start_date, end_date) # 30 x 30 m/px
        
        if drop_faulty_l7:
            # On May 31, 2003 the Scan Line Corrector on Landsat 7 failed, meaning that all images after will contain gaps.
            # I leave it up to the user whether to drop these images or keep them.
            # For more info, see "https://www.usgs.gov/landsat-missions/landsat-7"
            landsat_7 = landsat_7.filterDate(start_date, '2003-05-31')
        ...
        if collection == 5:
            ...
            # All collections
            landsat = landsat_5.merge(landsat_7).merge(landsat_8)
            
        if mask_clouds:
            # Whether to mask out cloudy pixels or keep them
            landsat = landsat.map(LandsatExporter.mask_qaclear)
            
        landsat = landsat.select(LandsatExporter.MS_BANDS)
            
        landsat = landsat.sort('system:time_start')
        
        return landsat
        
    def init_coll(name: str, filterpoly: ee.Geometry, start_date: str, end_date: str) -> ee.ImageCollection:
        """
        ...
        """
        img_col = ee.ImageCollection(name).filterBounds(filterpoly).filterDate(start_date, end_date)

        is_landsat_8 = name.startswith('LANDSAT/LC08')
        if is_landsat_8:
            return img_col.map(LandsatExporter.rename_l8) #.map(LandsatExporter.rescale_l8)
        else:
            return img_col.map(LandsatExporter.rename_l57) #.map(LandsatExporter.rescale_l57)
    ...
    def mask_qaclear(img: ee.Image) -> ee.Image:
        ...
        :return: ee.Image, input image with cloud-shadow, snow, cloud, and unclear
            pixels masked out
        """
        qam = LandsatExporter.decode_qamask(img)
