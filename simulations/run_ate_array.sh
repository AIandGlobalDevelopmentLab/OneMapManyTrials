...
#SBATCH --output=logs/ate_analysis_%A_%a.out   # %A = job ID, %a = array index
#SBATCH --error=logs/ate_analysis_%A_%a.err
#SBATCH -t 0-03:00:00      # Adjust the wall time as needed
...
apptainer exec $APPTAINER_IMG python run_simulation.py --random_state ${SLURM_ARRAY_TASK_ID} "$@"
